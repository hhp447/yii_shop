...
class OrderController extends CommonController
{
    ...
    public function actionCheck()//收银台页面的方法
    {
        if (Yii::$app->session['isLogin'] != 1) {//判断用户是否登录
            return $this->redirect(['member/auth']);
        }
        $orderid = Yii::$app->request->get('orderid');//获取传递过来的订单id
        $status = Order::find()->where('orderid = :oid', [':oid' => $orderid])->one()->status;
        if ($status != Order::CREATEORDER && $status != Order::CHECKORDER) {//判断该订单的状态
            return $this->redirect(['order/index']);
        }
        $loginname = Yii::$app->session['loginname'];//获取用户名或者用户邮箱
        $userid = User::find()->where('username = :name or useremail = :email', [':name' => $loginname, ':email' => $loginname])->one()->userid;//先找到登录人的ID
    ...
    public function actionAdd()//创建订单方法
    {
        if (Yii::$app->session['isLogin'] != 1) {
            return $this->redirect(['member/auth']);
        }
        ...
        try {
            if (Yii::$app->request->isPost) {
                ...
                $orderid = $ordermodel->getPrimaryKey();//获取主键ID
                foreach ($post['OrderDetail'] as $product) {
                    ...
                    Product::updateAllCounters(['num' => -$product['productnum']], 'productid = :pid', [':pid' => $product['productid']]);//商品的库存进行改变
                }
            }
            $transaction->commit();
        }catch(\Exception $e) {
    ...
    public function actionConfirm()//确认订单方法-更新order表的部分字段(addressid,expressid,status,amount)
    {
        //addressid, expressid, status, amount(orderid,userid)
        try {
            if (Yii::$app->session['isLogin'] != 1) {//判断当前用户是否登录
                return $this->redirect(['member/auth']);
            }
            if (!Yii::$app->request->isPost) {
                throw new \Exception();
            ...
            $loginname = Yii::$app->session['loginname'];
            $usermodel = User::find()->where('username = :name or useremail = :email', [':name' => $loginname, ':email' => $loginname])->one();//获取该用户
            if (empty($usermodel)) {
                throw new \Exception();
            }
            $userid = $usermodel->userid;
    ...
    public function actionPay()//支付宝支付方法
    {
        try{
            if (Yii::$app->session['isLogin'] != 1) {
                throw new \Exception();
